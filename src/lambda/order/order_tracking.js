...
exports.handler = async (event, context) => {
    try {
        ...
        const order_tracking_exists = await db.search_one(
            "order_shippings",
            "os_tracking_number",
            os_tracking_number
        )
        ...
        if (order_tracking_exists.length === 0) {
            return handler.returner(
                [
                    ...
                    {
                        message:
                            "Tracking number does not exist, please provide correct number and try again",
                    },
                ],
                api_name,
                404
        ...
        const order_shipping_status_exists = await db.search_one(
