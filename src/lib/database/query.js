...
module.exports = {
  ...
  join_two_tables_and_search_one: async(tableOne, tableTwo, condition, columns, search_column, data) => {
    const result = await connection.query(
      `SELECT ${columns.join(
        ", "
      )} FROM ${tableOne} JOIN ${tableTwo} ON ${tableOne}.${condition}=${tableTwo}.${condition} WHERE ${search_column} = ?`,
      [data]
    );
    return result;
  }
